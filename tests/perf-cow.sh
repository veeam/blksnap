...
if [ ! -f ${SOURCE_FILE} ]
then
	dd if=/dev/urandom of=${SOURCE_FILE} count=$((7 * 1024)) bs=1M oflag=direct status=none
fi
...
# echo "press a key..."
# read -n 1
perf record -ag dd if=${SOURCE_FILE} of=${TARGET_FILE} bs=1M iflag=direct oflag=direct
